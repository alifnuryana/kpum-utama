...
@section('title', 'Edit Kandidat UKM')
@section('container')
    ...
    <div class="container mb-5">
        <form action="{{ route('ukm.update', $ukm->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="text" hidden name="id" value="{{ $ukm->id }}">
            <div class="mb-3">
                <label for="name" class="form-label">Nama Kandidat</label>
                <input type="text" value="{{ old('name') ? old('name') : $ukm->name }}"
            ...
            <div class="mb-3">
                ...
                <input type="number" value="{{ old('npm') ? old('npm') : $ukm->npm }}"
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="position">
                    <option @if ($ukm->poisiton == 'Ketua')
                        selected
                        @endif value="Ketua">Ketua</option>
                    <option @if ($ukm->position == 'Wakil')
                        selected
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="major_id">
                    ...
                    @foreach ($majors as $major)
                        <option @if (old('major_id') == $major->id || $ukm->major_id == $major->id)
                            selected
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="team_id">
                    ...
                    @foreach ($teams as $team)
                        <option @if (old('team_id') == $team->id || $ukm->team_id == $team->id)
                            selected
                            @endif value="{{ $team->id }}">
                            {{ $team->name }}</option>
                    @endforeach
